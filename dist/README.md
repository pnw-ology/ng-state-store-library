# state-store-library
[![Build Status](https://travis-ci.org/robisim74/state-store-library.svg?branch=master)](https://travis-ci.org/robisim74/state-store-library)
...
Get the [Changelog](https://github.com/robisim74/state-store-library/blob/master/CHANGELOG.md).
...
2. Rename `state-store-library` and `StateStoreLibrary` everywhere to `my-library` and `myLibrary`.
...
1. `package.json`
    ...
    * `"main": "./bundles/state-store-library.umd.js"` legacy module format 
    * `"module": "./bundles/state-store-library.es5.js"` flat _ES_ module, for using module bundlers such as _Rollup_ or _webpack_: 
    [package module](https://github.com/rollup/rollup/wiki/pkg.module)
    * `"es2015": "./bundles/state-store-library.js"` _ES2015_ flat _ESM_ format, experimental _ES2015_ build
...
4. `rollup.config.js` file used by _Rollup_
    ...
    * `moduleName: 'ng.StateStoreLibrary'` defines the global namespace used by _JavaScript_ apps
