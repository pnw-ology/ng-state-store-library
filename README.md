...
A library for adding a state store to Angular (2+) Applications. Built on scaffolding generated by angular quickstart library.

-------------------------------------------

For development of this library, the "angular quickstart lib" has been used. The note below detail usage of that library.
...
[travis-badge]: https://travis-ci.org/filipesilva/angular-state-store-service.svg?branch=master
[travis-badge-url]: https://travis-ci.org/filipesilva/angular-state-store-service
...
1. Create a project folder (you can call it `state-store-service` and rename it later).
...
```
git clone https://github.com/angular/state-store-service.git state-store-service
cd angular-state-store-service
npm install
npm start
```
...
### Download
[Download the QuickStart Library seed](https://github.com/filipesilva/angular-state-store-service/archive/master.zip)
...
<table width="100%">
  ...
  <tr>
    ...
    <td>
      A demo <code>NgModule</code> that imports the Library <code>StateStoreModule</code>.
    </td>
  </tr>
  <tr>
    <td>
  ...
  <tr>
    ...
    <td>
      The library's main <code>NgModule</code>, <code>StateStoreModule</code>.
    </td>
  </tr>
  <tr>
    <td>
...
Use your editor to search the project for all instances of `angular-state-store-service` and change it
